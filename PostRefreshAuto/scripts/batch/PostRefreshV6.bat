...
@echo -----------------------------------------------------------------------------------
REM sfdx force:mdapi:deploy -d .\metaoutput\ -u %UserName% -w -1
@echo -----------------------------------------------------------------------------------
...
sfdx force:data:soql:query -q "SELECT ID,Name,ccSPGAddresses__c,toSPGAddresses__c FROM Strategic_Partner_Group_Setting__c" -r csv >> ./metadata/setting/Strategic_Partner_Group_Setting.csv
cd metadata/setting
